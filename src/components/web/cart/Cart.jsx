...
export default function Cart() {
  ...
  const { getCartContext, removeItemContext, clearcart,incraseQuantityContext,decraseQuantityContext } = useContext(CartContext);
  const getCart = async () => {
  ...
  const clearcartall = async (productId) => {
    const clear = await clearcart(productId);
    return (clear);

  }
  const incraseQuantity =async(productId)=>{
    const resal =await incraseQuantityContext(productId);
    return resal;
  }
  const decraseQuantity =async(productId)=>{
    const resal =await decraseQuantityContext(productId);
    return resal;
  }
  ...
  return (
    <div className="cart">
      <div className="container">
        <button href="#" onClick={() => clearcartall(products._id)} style={{ backgroundColor: 'GrayText' }}> clear cart</button>
        <div className="row">
            ...
            <div className="products" id="products">
              ...
              {data?.products ? (
                data.products.map((products) =>
                  <div className="item" key={products._id}>
                    <div className="product-info">
                      <img src={products.details.mainImage.secure_url} />
                      <div className="product-details">
                        <h2>{products.details.name}</h2>
                        <span>Color:black</span>
                        <a href="#" onClick={() => removeItem(products.details._id)}>
                    ...
                    <div className="quantity">
                      <button onClick={() => decraseQuantity(products.details._id)}>
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          width={16}
                          height={17}
                      ...
                      <button onClick={() => incraseQuantity(products.details._id)}>
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          width={16}
                          height={17}
                    ...
                    <div className="price">${products.details.price}</div>
                    <div className="subtotal">${products.details.price * products.quantity}</div>
